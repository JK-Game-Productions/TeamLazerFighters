...
public class GameObject
{
	...
	*/
	...
	/** This function rotates the game object around the Y axis based off time and direction */
	...
	public void gyaw(float diff, boolean left){
		if(left)
			//this.setLocalRotation(this.getWorldRotation().mul(new Matrix4f().rotationY(-0.005f)));
			this.setLocalRotation((new Matrix4f().rotationY(1.0f * diff)).mul(this.getWorldRotation()));
		else
			//this.setLocalRotation(this.getWorldRotation().mul(new Matrix4f().rotationY(0.005f)));
			this.setLocalRotation((new Matrix4f().rotationY(-1.0f * diff)).mul(this.getWorldRotation()));
	}
	/** This function rotates the game object around the X Axis based off time and direction*/
	public void pitch(float diff, boolean up) {
		Vector4f oldRot = new Vector4f(1f,0f,0f,1f).mul(new Matrix4f(this.getWorldRotation()));
		if (up)
			//this.setLocalRotation(this.getWorldRotation().mul(new Matrix4f().rotation(-0.005f,new Vector3f(oldRot.x(), oldRot.y(), oldRot.z()))));
			this.setLocalRotation((new Matrix4f().rotation(-1.0f * diff,new Vector3f(oldRot.x(), oldRot.y(), oldRot.z()))).mul(this.getWorldRotation()));
		else
			//this.setLocalRotation(this.getWorldRotation().mul(new Matrix4f().rotation(0.005f,new Vector3f(oldRot.x(), oldRot.y(), oldRot.z()))));
			this.setLocalRotation((new Matrix4f().rotation(1.0f * diff,new Vector3f(oldRot.x(), oldRot.y(), oldRot.z()))).mul(this.getWorldRotation()));
	...
	public void changeScale(float factor){
		this.setLocalScale((new Matrix4f()).scaling((new Vector3f(factor,factor,factor)).mul(this.getWorldScale().getScale(new Vector3f()))));
	}
	...
	/** This function moves a Game Object along its Z axis based on the objects move factor */
